...
function updateMyPostsTab() {
    fetch('/feedback/latest')
        ...
        .then(latestFeedback => {
            ...
            // Check if the feedback list container exists
            ...
            // Prepend the new feedback to the feedback list container
...
function formatFeedbackHtml(feedback) {
    ...
    return `
<div id="feedback-item-${feedback.id}" class="d-flex flex-start">
   <img class="rounded-circle shadow-1-strong me-3" src="data:image/jpeg;base64,${feedback.decompressedImageBase64}" alt="avatar" width="60" height="60"/>
   <div>
      <div class="name-date d-flex align-items-center mb-3">
         ...
         <a href="#edit" class="link-muted edit-icon" style="color: green; margin-left: 28rem" data-feedback-id="${feedback.id}" data-student-id="${feedback.student.id}" data-student-name="${feedback.student.firstName} ${feedback.student.lastName}" data-company-name="${feedback.companyName}" data-comment="${feedback.comment}" data-rating="${feedback.rating}">
         <i class="fas fa-pencil-alt ms-2"></i>
         </a>
         <a href="#" class="link-muted delete-icon" style="color: red; margin-right: 1rem;" data-feedback-id="${feedback.id}" data-student-id="${feedback.student.id}" onclick="confirmDeleteFeedback(${feedback.id})">
            <svg class="icon-20 delete-icon ms-2" width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor">
               ...
               <path d="M17.4406 6.23973C16.6556 6.23973 15.9796 5.68473 15.8256 4.91573L15.5826 3.69973C15.4326 3.13873 14.9246 2.75073 14.3456 2.75073H10.1126C
                  9.53358 2.75073 9.02558 3.13873 8.87558 3.69973L8.63258 4.91573C8.47858 5.68473 7.80258 6.23973 7.01758 6.23973" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
            </svg>
         </a>
         <p class="mb-0">
            ${statusBadgeHtml}
      ...
      <div class="d-flex align-items-center mb-1">
         <div style="margin-left: 36rem">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="margin-bottom: .5rem">
               <path fill="#ffd43b" d="M9.153 5.408C10.42 3.136 11.053 2 12 2c.947 0 1.58 1.136 2.847 3.408l.328.588c.36.646.54.969.82 1.182c.28.213.63.292 1.33.45l.636.144c2.46.557 3.689.835 3.982 1.776c.292.94-.546 1.921-2.223 3.882l-.434.507c-.476.557-.715.836-.822 1.18c-.107.345-.071.717.
                  001 1.46l.066.677c.253 2.617.38 3.925-.386 4.506c-.766.582-1.918.051-4.22-1.009l-.597-.274c-.654-.302-.981-.452-1.328-.452c-.347 0-.674.15-1.328.452l-.596.274c-2.303 1.06-3.455 1.59-4.22 1.01c-.767-.582-.64-1.89-.387-4.507l.066-.676c.072-.744.108-1.116 0-1.46c-.106-.345-.345-.624-.821-1.18l-.434-.508c-1.677-1.96-2.515-2.941-2.223-3.882c.293-.941 1.523-1.22 3.983-1.776l.636-.144c.699-.158 1.048-.237 1.329-.45c.28-.213.46-.536.82-1.182z"/>
            </svg>
         </div>
      </div>
